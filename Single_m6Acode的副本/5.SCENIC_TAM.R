...
cellInfo <-  phe[,c('NMFcluster','nCount_RNA' ,'nFeature_RNA' )]
colnames(cellInfo)=c('CellType', 'nGene' ,'nUMI')
head(cellInfo)
table(cellInfo$CellType)
Idents(sce)<-cellInfo$CellType
...
aucsmenas<-aggregate(aucss[,c(1:38)],list(aucss[,39]),mean)#########根据最后确定的TF数量进行构建 heatmap
...
pdf("TF_scenic_NMF_Macrophage.pdf",height = 3.84,width=3.52)
