...
save(methyScorelist,file="CD8_signature_Scoreforbulksequence.Rdata")
load("CD8_signature_Scoreforbulksequence.Rdata")###

save(methyScorelist,file="Treg_signature_Scoreforbulksequence.Rdata")
load("Treg_signature_Scoreforbulksequence.Rdata")
#for RFS
b<-c()
...
for(set in names(exp)[c(1:14)]){
  ...
  colnames(gsva_es)<-c('geo_accession',names(ProSig))
  myphe<-phe[[set]]
  ...
  if(nrow(rt)>10){
    for (sig in variables){
      ...
      if (!dir.exists("Sc_Treg_NMF_COHORT")){
        dir.create("./Sc_Treg_NMF_COHORT")
      }
      if(p.val<0.05){
        pdf(paste0("./Sc_Treg_NMF_COHORT/",set,sig,HR,"RFS_",".pdf"),width = 6.49, height = 6.58,onefile = F )
...
write.csv(colorectal_cox.sig,file="colorectal_cox.sig_RFS_Treg.csv")###
...
#rownames(NMFcoxmeta)<-c("C1","C4")
rownames(NMFcoxmeta)<-c("CD8_C1","CD8_C4")
rownames(NMFcoxmeta)<-c("Treg_C1","Treg_C4")
head(NMFcoxmeta)
library(DT)
...
######
tabletext <- cbind(c("\nSubgroup names",NA,rownames(rt),NA),
                   c("Hazard Ratio\n(95% CI)",NA, 
                     paste0(format(rt$HR,nsmall=2),
                            " (",format(rt$low,nsmall = 2),"-",format(rt$up,nsmall = 2),")",sep=""),NA),
                   c("p-value",NA,rt$pvalue,NA))
pdf(paste0(cellnames[1],"forestplot_RFS.pdf"),width = 8,height = 5,onefile=FALSE)
...
forestplot(labeltext=tabletext, #
           ...
           hrzl_lines=list("3" = gpar(lwd=2, col="black"),#
                           ...
                           "6" = gpar(lwd=2, col="black")),#"nrow(rt)+5
...
for(set in names(exp)[c(1,4:9,11:14)]){
  ...
  if(nrow(rt)>10){
    for (sig in variables){
      ...
      print(paste0(set,sig," p= ",round(p.val,2)," ",HR))
      # pdf(paste("plot", i, ".pdf", sep = ""))
      if (!dir.exists("Sc_Treg_NMF_COHORT_OS")){
        dir.create("./Sc_Treg_NMF_COHORT_OS")
      }
      if(p.val<0.05){
        pdf(paste0("./Sc_Treg_NMF_COHORT_OS/",set,sig,HR,"OS_",".pdf"),width = 6.49, height = 6.58,onefile = F )
        print(ggsurvplot(kmfit,#surv.median.line = "hv", # Add medians survival
                         # Change legends: title & labels
                         main = "Survival curve",
                         legend.title = paste0(set,"-",sig),
...
#colorectal_cox.sig<-colorectal_cox.sig[colorectal_cox.sig$P.value<0.05,]
write.csv(colorectal_cox.sig,file="colorectal_cox.sig_OS_CD8.csv")
write.csv(colorectal_cox.sig,file="colorectal_cox.sig_OS_Treg.csv")
######################################################################
...
datatable(NMFcoxmeta, 
          ...
          options = list(dom = 'Bfrtip', 
                         buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
write.csv(NMFcoxmeta,file="colorectal_cox.sig_OS_CD8_meta_results.csv")####
######
######forest
library("forestplot")
library("magrittr")
