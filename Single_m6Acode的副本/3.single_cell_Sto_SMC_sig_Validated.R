...
save(methyScorelist,file="CAF_signature_Scoreforbulksequence.Rdata")
load("CAF_signature_Scoreforbulksequence.Rdata")
#for RFS
b<-c()
...
for(set in names(exp)[c(1:14)]){
  gsva_es<-methyScorelist[[set]]
  colnames(gsva_es)<-c('geo_accession',"C1", "C2" ,"C3" , "C4")
  myphe<-phe[[set]]
  ...
  if(nrow(rt)>10){
    for (sig in variables){
      ...
      if (!dir.exists("Sc_CAF_NMF_COHORT")){
        dir.create("./Sc_CAF_NMF_COHORT")
      }
      if(p.val<0.05){
        pdf(paste0("./Sc_CAF_NMF_COHORT/",set,sig,HR,"RFS_",".pdf"),width = 6.49, height = 6.58,onefile = F )
...
write.csv(colorectal_cox.sig,file="colorectal_cox.sig_RFS_CAF.csv")###
...
setnames<-c("C1", "C2" ,"C3" , "C4")
NMFcoxmeta<-c()
NMFcoxlist<-list()
for (set in setnames) {NMFcox.i<-NMFcox[[set]][-14,]
...
rownames(NMFcoxmeta)<-c("C1", "C2" ,"C3","C4")
head(NMFcoxmeta)
library(DT)
...
datatable(NMFcoxmeta, 
          ...
          options = list(dom = 'Bfrtip', 
                         buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
write.csv(NMFcoxmeta,file="colorectal_cox.sig_RFS_CAF_meta_results.csv")###
save(NMFcoxmeta,colorectal_cox.sig,file="colorectal_cox.sig_RFS_CAFmeta.Rdata")
##
######forest
library("forestplot")
library("magrittr")
...
forestplot(labeltext=tabletext, #
           ...
           hrzl_lines=list("3" = gpar(lwd=2, col="black"),#
                           ...
                           "8" = gpar(lwd=2, col="black")),#"nrow(rt)+5
...
colorectal_cox.sig<-c()
for(set in names(exp)[c(1,4:9,11:14)]){
  gsva_es<-methyScorelist[[set]]
  colnames(gsva_es)<-c('geo_accession',"C1", "C2" ,"C3","C4")
  myphe<-phe[[set]]
  #colnames(PANCAN.survival)
  ...
  if(nrow(rt)>10){
    for (sig in variables){
      ...
      print(paste0(set,sig," p= ",round(p.val,2)," ",HR))
      # pdf(paste("plot", i, ".pdf", sep = ""))
      if (!dir.exists("Sc_CAF_NMF_COHORT_OS")){
        dir.create("./Sc_CAF_NMF_COHORT_OS")
      }
      if(p.val<0.05){
        pdf(paste0("./Sc_CAF_NMF_COHORT_OS/",set,sig,HR,"OS_",".pdf"),width = 6.49, height = 6.58,onefile = F )
        print(ggsurvplot(kmfit,#surv.median.line = "hv", # Add medians survival
                         # Change legends: title & labels
                         main = "Survival curve",
                         legend.title = paste0(set,"-",sig),
...
#colorectal_cox.sig<-colorectal_cox.sig[colorectal_cox.sig$P.value<0.05,]
write.csv(colorectal_cox.sig,file="colorectal_cox.sig_OS_CAF.csv")
######################################################################
...
NMFcoxmeta<-c()
NMFcoxlist<-list()
for (set in setnames) {NMFcox.i<-NMFcox[[set]][-c(8,9),]
res.rma_each<- metafor::rma(yi = NMFcox.i[,4], sei = NMFcox.i[,5],method="FE")
results<-round(c(res.rma_each$pval,exp(res.rma_each$b),exp(res.rma_each$ci.lb),exp(res.rma_each$ci.ub)),3)
NMFcoxlist[[set]]<-results;
#results<-cbind(set,results)
...
datatable(NMFcoxmeta, 
          ...
          options = list(dom = 'Bfrtip', 
                         buttons = c('copy', 'csv', 'excel', 'pdf', 'print')))
write.csv(NMFcoxmeta,file="colorectal_cox.sig_OS_CAF_meta_results.csv")####
######
######forest
library("forestplot")
library("magrittr")
...
tabletext <- cbind(c("\nSubgroup Cell",NA,rownames(rt),NA),
                   c("Hazard Ratio\n(95% CI)",NA, 
                     paste0(format(rt$HR,nsmall=2),
                            " (",format(rt$low,nsmall = 2),"-",format(rt$up,nsmall = 2),")",sep=""),NA),
                   c("p-value",NA,rt$pvalue,NA))
pdf(paste0("CAF_","forestplot_OS.pdf"),width = 8,height = 5,onefile=FALSE)
forestplot(labeltext=tabletext, #
           mean=c(NA,NA,rt$HR,NA),#HR
           lower=c(NA,NA,rt$low,NA), #95%
           upper=c(NA,NA,rt$up,NA),#95%
...
###########################select the genes for Km curves in the top 15 genes
head(single_dat_sce.markers_top5)
######################
single_dat_sce.markers_top5<-sce.markers[sce.markers$cluster==2,]$gene
#gene.cox<-list()
gene.coxs<-c()
for(set in names(exp)[c(1,4:9,11:14)]){
  myexp<-exp[[set]]
  myphe<-phe[[set]]
  for (gene in sce.markers[sce.markers$cluster==2,]$gene){
    if(is.element(gene,rownames(myexp))){
      genexp<-myexp[gene,] %>% t() %>% data.frame()
      genexp<-cbind(geo_accession=colnames(myexp),genexp)
      rt<-merge(genexp,myphe,by="geo_accession")
      rt$OS.time<-as.numeric(as.character(rt$OS_time))
      rt$OS<-as.numeric(as.character(rt$OS))
      rt<-rt[which(rt$OS!='NA'),]
      rt<-data.frame(rt)
      if(nrow(rt)>10){
        ...
        sur.cut<-surv_cutpoint(rt,time = "OS.time", event = "OS", variables = gene)
        rt$score_group=ifelse(rt[,gene]>(sur.cut$cutpoint[,1]),1,0)##bug 
        #rt$score_group=rt[,gene]>median(rt[,gene])
        #rt$score_group=rt[,gene]>summary(rt[,gene])[5]
        Gcox1<-coxph(Surv(OS.time,OS)~score_group,data=rt)
        GSum<-summary(Gcox1)
        HR<-round(GSum$coefficients[,2],3)
        Pvalue<-round(GSum$coefficients[,5],3)
        ...
        low<-round(GSum$conf.int[,3],3)
        up<-round(GSum$conf.int[,4],3)
        cox.p<-data.frame('Characteristics'= gene,
                          'Hazard Ratio'= HR,
                          'CI95'=CI,
                          "coeff"=coeff,
                          "se"=se,
                          ...
                          "set"=set,
                          "numberofpatients"=nrow(rt))
        gene.coxs=rbind(gene.coxs,cox.p)
        message(gene,"cox regression is Done!")
        kmfit<- survfit(Surv(OS.time,OS)~score_group,data=rt)
        data.survdiff <- survdiff(Surv(OS.time,OS) ~ score_group,data=rt)
        p.val = 1 - pchisq(data.survdiff$chisq, length(data.survdiff$n) - 1)
        HR = (data.survdiff$obs[2]/data.survdiff$exp[2])/(data.survdiff$obs[1]/data.survdiff$exp[1])
        up95 = exp(log(HR) + qnorm(0.975)*sqrt(1/data.survdiff$exp[2]+1/data.survdiff$exp[1]))
        low95 = exp(log(HR) - qnorm(0.975)*sqrt(1/data.survdiff$exp[2]+1/data.survdiff$exp[1]))
        ...
        print(paste0(set,gene," p= ",round(p.val,2)," ",HR))
        # pdf(paste("plot", i, ".pdf", sep = ""))
        if (!dir.exists("Sc_CAF_NMF")){
          dir.create("./Sc_CAF_NMF")
        }
        if(p.val<0.05){
          pdf(paste0("./Sc_CAF_NMF/",set,gene,HR,"OS_",".pdf"),width = 6.49, height = 6.58,onefile = F )
          print(ggsurvplot(kmfit,#surv.median.line = "hv", # Add medians survival
                           # Change legends: title & labels
                           main = "Survival curve",
                           legend.title = paste0(set,"-",gene),
                           legend.labs = c("Low","High"),
                           #xlim = c(0, 200),
                           xlab="Time(months)",
                           ylab="R-F Survival probability",
...
gene.coxs<-gene.coxs[gene.coxs$P.value<0.05,]##
gene.coxs_set<-gene.coxs[gene.coxs$set=="GSE17538",]
